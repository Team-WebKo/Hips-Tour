...
class ReviewServiceImplTest {
    ...
    void setUp() throws Exception {
        place = new Place("테스트장소", "서울시 어딘가", "상세주소", null, null);
        setPrivateField(place, "placeId", 1);

        userInfo = new UserInfo();
        setPrivateField(userInfo, "userId", 101L);
        setPrivateField(userInfo, "email", "test@example.com");
        setPrivateField(userInfo, "nickName", "테스터");
    }
    ...
    void getReviews_firstPage() {
        ...
        assertThat(result.get(1).getHeadText()).isEqualTo("제목2");
        assertThat(result.get(1).getBodyText()).isEqualTo("내용2");
    ...
    @Test
    @DisplayName("ex) offset=2, limit=2 → 다음 페이지(추가 2개)")
    void getReviews_nextPage() {
        ...
        assertThat(result.get(0).getHeadText()).isEqualTo("제목3");
        assertThat(result.get(0).getBodyText()).isEqualTo("내용3");
    ...
    @Test
    @DisplayName("존재하지 않는 placeId 로 요청 시 PlaceNotFoundException")
    void getReviews_placeNotFound() {
        int invalidPlaceId = 999;
    ...
    @Test
    @DisplayName("리뷰가 존재하지 않으면 ReviewNotFoundException")
